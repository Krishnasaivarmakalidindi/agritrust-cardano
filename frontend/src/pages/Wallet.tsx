  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8 space-y-8">
      ...
      <div>
        ...
        <p className="text-sm text-slate-400 mt-1">Manage simulated Ada test assets and track locked smart contract escrow funds</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="glass-card rounded-2xl p-6 relative overflow-hidden flex flex-col justify-between h-48 border-l-2 border-emerald-500">
          ...
          <div>
            <div className="flex items-baseline space-x-1.5">
              <span className="text-3xl font-extrabold text-white">₳ {wallet.balance.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
              <span className="text-xs text-slate-400 font-semibold uppercase">ADA</span>
            </div>
        ...
        <div className="glass-card rounded-2xl p-6 relative overflow-hidden flex flex-col justify-between h-48 border-l-2 border-orange-500">
          ...
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold uppercase tracking-wider text-slate-400">Locked in Escrow</span>
          ...
          <div>
            <div className="flex items-baseline space-x-1.5">
              <span className="text-3xl font-extrabold text-white">₳ {wallet.locked_balance.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
              <span className="text-xs text-slate-400 font-semibold uppercase">ADA</span>
            </div>
            <p className="text-xs text-slate-400 mt-1">≈ ₹{(wallet.locked_balance * 35).toLocaleString(undefined, { maximumFractionDigits: 2 })} INR</p>
          </div>
          <div className="text-[10px] text-slate-500">
        ...
        <div className="glass-card rounded-2xl p-6 flex flex-col justify-between h-48">
          <div>
            <span className="text-xs font-bold uppercase tracking-wider text-slate-400">Wallet Details</span>
            <div className="mt-2.5 flex items-center justify-between rounded-xl bg-slate-900 px-3.5 py-2.5 border border-slate-800">
              <div className="min-w-0 flex-1">
                <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest leading-none">Wallet Address</p>
                <p className="font-mono text-xs text-white truncate mt-1.5">{wallet.address}</p>
              </div>
              <button 
                onClick={copyAddress}
                className="ml-3 flex h-8 w-8 items-center justify-center rounded-lg bg-slate-850 hover:bg-slate-800 text-slate-400 hover:text-white transition"
              >
      ...
      <div className="glass-card rounded-2xl p-6 shadow-xl">
        <h2 className="font-display text-lg font-bold text-white mb-6">On-Chain Transaction Log</h2>

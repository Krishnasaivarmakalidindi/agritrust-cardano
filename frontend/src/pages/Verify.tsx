...
import { ShieldCheck, Calendar, ArrowRight, UserCheck, ArrowLeft, Database, Globe, Leaf } from 'lucide-react';
...
export const Verify: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchVerificationData = async () => {
      setLoading(true);
      try {
        const orderData = await dbService.getOrder(id);
        if (orderData) {
          setOrder(orderData);
          const contractData = await dbService.getContractForOrder(orderData.id);
          setContract(contractData);

          // Get matching blocks from the ledger
          const logs = await dbService.getBlockchainLogs();
          const matched = logs.filter(log => log.data && log.data.orderId === orderData.id);
  ...
  if (!order) {
    return (
      <div className="mx-auto max-w-md px-4 py-16 text-center space-y-4">
        ...
        <p className="text-sm text-slate-450">The requested trade ID does not match any smart contract audits on this network.</p>
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-8 sm:px-6 lg:px-8 space-y-8 relative">
      ...
      <div className="glass-card rounded-3xl p-6 border-slate-855 text-center space-y-5 relative z-10 overflow-hidden">
        ...
        <div className="space-y-1.5">
          <span className="rounded-full bg-emerald-500/10 border border-emerald-500/20 px-3 py-1 text-[10px] font-bold text-emerald-400 uppercase tracking-widest inline-block">
            Blockchain Origin Verified
          </span>
          <h1 className="font-display text-2xl font-black text-white">Supply Chain Verification Receipt</h1>
          <p className="text-xs text-slate-400">Order ID: #{order.id} • Cardano Preview Testnet Audited</p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3 border-t border-slate-900 pt-5 text-xs text-slate-400">
          <div>
            <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest block">Settlement Asset</span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 relative z-10">
        ...
        <div className="glass-card rounded-2xl p-5 border-slate-855 space-y-4">
          ...
          <div className="space-y-3 text-xs">
            ...
            <div className="flex justify-between">
              <span className="text-slate-450">Category / Grade:</span>
              <span className="text-slate-200">{order.product?.category} • {order.product?.grade}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-slate-450">Shipment Volume:</span>
            ...
            <div className="border-t border-slate-900 pt-3.5 space-y-2">
              ...
              <p className="text-[10px] text-slate-400 font-mono pl-4 truncate">WALLET: {order.farmer_id}</p>
              <p className="text-[10px] text-emerald-400 font-bold pl-4">Trust Reputation: {order.farmer?.trust_score}%</p>
            </div>
          </div>
        </div>
        ...
        <div className="glass-card rounded-2xl p-5 border-slate-855 space-y-4">
          ...
          <div className="space-y-3 text-xs">
            <div className="flex justify-between">
              <span className="text-slate-450">Script Engine:</span>
              <span className="text-slate-200">Plutus Validator V1 (Aiken)</span>
            </div>
            <div className="flex justify-between">
              <span className="text-slate-450">Validator Hash:</span>
            ...
            <div className="border-t border-slate-900 pt-3.5 space-y-2">
              ...
              <p className="text-[10px] text-slate-400 font-mono pl-4 truncate">WALLET: {order.buyer_id}</p>
              <p className="text-[10px] text-emerald-400 font-bold pl-4">Trust Reputation: {order.buyer?.trust_score}%</p>
            </div>
          </div>
        </div>
              ...
              <div key={block.id} className="flex flex-col sm:flex-row justify-between items-start sm:items-center rounded-xl bg-slate-900/60 p-3 border border-slate-850 text-xs gap-2">
                <div className="space-y-0.5">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="font-bold text-white">{block.action.replace(/_/g, ' ')}</span>
                  </div>
                  <p className="font-mono text-[9px] text-slate-500">TX HASH: {block.tx_hash}</p>
                </div>
